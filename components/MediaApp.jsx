...
// Concurrency limiter: max 4 simultaneous fetchDirectory calls
...
const Thumbnail = ({ item, index = 0 }) => {
  ...
  // Only start fetching once the thumbnail enters the viewport
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Stagger only if many items; visible items already throttled by IntersectionObserver
    const delay = index < 6 ? 0 : Math.min((index - 5) * 30, 500);

    const timer = setTimeout(() => {
      if (!isMounted) return;
      enqueueRequest(() => fetchDirectory(item.href)).then(async (data) => {
        ...
        if (imgs.length === 0) {
          ...
          if (subfolders.length > 0) {
            const subDataList = await Promise.all(
              subfolders.map(sub => enqueueRequest(() => fetchDirectory(sub.href)).catch(() => null))
            );
            if (!isMounted) return;
        ...
        const finalImgs = imgs.slice(0, 100);
  ...
  }, [item.href, item.isFolder, index, visible]);
    ...
    return (
      <div ref={containerRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        <AnimatePresence mode="wait">
          <motion.img
            key={activeImg}
            src={`/api/proxy?url=${encodeURIComponent(images[activeImg])}`}
            alt={item.title}
            loading="lazy"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
  ...
  return <div ref={containerRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{item.isFolder ? <Folder size={40} /> : <Play size={40} />}</div>;
};


const heroCache = new Map();

const HeroSlider = ({ onPlay, featuredPath = '/DHAKA-FLIX-7/English Movies/' }) => {
  ...
  useEffect(() => {
    ...
    fetchDirectory(featuredPath).then(async (items) => {
      ...
      const folders = items.filter(f => f.isFolder).slice(0, 10);
      let results = [];
      if (folders.length > 0) {
        results = await Promise.all(
          folders.map(async (folder) => {
            try {
              ...
              let img = files.find(f => !f.isFolder && /\.(jpg|jpeg|png|webp|gif)$/i.test(f.href));
              let video = files.find(f => !f.isFolder && /\.(mp4|mkv|avi|webm|m4v)$/i.test(f.href));
              
              if (!img && !video) {
                const subfolders = files.filter(f => f.isFolder);
                if (subfolders.length > 0) {
                  const subFiles = await fetchDirectory(subfolders[0].href).catch(() => []);
                  img = subFiles.find(f => !f.isFolder && /\.(jpg|jpeg|png|webp|gif)$/i.test(f.href)) || img;
                  video = subFiles.find(f => !f.isFolder && /\.(mp4|mkv|avi|webm|m4v)$/i.test(f.href)) || video;
                }
              }
  ...
  return (
    <div className="hero-slider-container" style={{ position: 'relative', width: '100%', borderRadius: '20px', overflow: 'hidden', marginBottom: '2.5rem', boxShadow: '0 8px 40px rgba(0,0,0,0.6)', background: '#0a0d18' }}>
      <AnimatePresence mode="wait">
        <motion.div
          key={activeIdx}
        ...
        >
          {slide.img ? (
            <img src={`/api/proxy?url=${encodeURIComponent(slide.img)}`} alt={slide.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
...
const EnhancedVideoPlayer = ({ movie }) => {
  ...
  const mediaUrl = `/api/proxy?url=${encodeURIComponent(getFullUrl(movie.href))}`;
...
const App = () => {
  ...
  async function loadDirectory(path, retries = 2) {
    setLoading(true);
    setError(null);
    try {
  ...
  const handleFileClick = (item) => {
    ...
    if (isVideo) {
      ...
      window.open(`/api/proxy?url=${encodeURIComponent(getFullUrl(item.href))}`, '_blank');
    }
  };
      ...
      <main className="main-content">
        ...
        <nav className="navbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', color: 'var(--text-secondary)', alignItems: 'center', flex: 1, justifyContent: 'flex-end' }}>
            {/* Search bar */}
            <div
              className={`search-bar ${searchQuery ? 'has-text' : ''}`}
            ...
            >
              ...
              />
              {searchQuery && (
                <button onClick={(e) => { e.stopPropagation(); setSearchQuery(''); }} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', padding: 0 }}>✕</button>
              )}
            </div>
            <button
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              >
                <Bell size={20} />
                {notifications.length > 0 && <span style={{ position: 'absolute', top: -2, right: -2, width: '8px', height: '8px', background: 'var(--danger-color)', borderRadius: '50%' }}></span>}
              </button>
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    className="glass"
                    style={{
                      position: 'absolute', top: '150%', right: 0, width: '320px', maxWidth: '90vw', padding: '1rem',
                      display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 100000,
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--surface-border)', paddingBottom: '0.5rem', marginBottom: '0.25rem' }}>
                      ...
                      <span onClick={() => setNotifications([])} style={{ fontSize: '0.75rem', color: 'var(--accent-color)', cursor: 'pointer', fontWeight: 600 }}>Mark all read</span>
                    </div>
                    {notifications.length > 0 ? (
                      notifications.map(n => (
                        <div key={n.id} onClick={() => setNotifications(prev => prev.filter(item => item.id !== n.id))} style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', padding: '0.5rem', borderRadius: '8px', cursor: 'pointer', transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = 'var(--surface-border)'} onMouseOut={e => e.currentTarget.style.background = 'transparent'}>
                      ...
                      <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>No new notifications</div>
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            {user ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  <img
                    ...
                    alt="User"
                    style={{ width: '32px', height: '32px', minWidth: '32px', minHeight: '32px', borderRadius: '50%', border: '2px solid var(--accent-color)', objectFit: 'cover', display: 'block', flexShrink: 0 }}
                  />
                </div>
                <button
              ...
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', position: 'relative' }}>
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: 'var(--text-primary)', fontWeight: 600, cursor: 'pointer', padding: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
                >
                  Log In
                </button>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'var(--accent-color)', border: 'none', color: '#fff', fontWeight: 600, cursor: 'pointer', padding: '0.5rem 1.25rem', borderRadius: '50px', boxShadow: '0 4px 15px rgba(59,130,246,0.4)', transition: 'all 0.2s' }}
                >
                  Sign Up
                </button>
                ...
                <AnimatePresence>
                  {authMenuOpen && (
                    <motion.div
                      ...
                      style={{
                        position: 'absolute', top: '120%', right: 0, width: '160px', padding: '0.5rem',
                        display: 'flex', flexDirection: 'column', gap: '0.25rem', zIndex: 100,
                        boxShadow: '0 10px 40px rgba(0,0,0,0.5)', borderRadius: '12px'
                      }}
                    >
                      <button
                        onClick={() => { setAuthMenuOpen(false); setAuthModalIsLogin(true); setAuthModalOpen(true); }}
                        style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: 'var(--text-primary)', fontWeight: 600, padding: '0.75rem', width: '100%', borderRadius: '8px', cursor: 'pointer', textAlign: 'left' }}
                      >
                        <LogIn size={16} /> Log In
                      </button>
                      <button
                        onClick={() => { setAuthMenuOpen(false); setAuthModalIsLogin(false); setAuthModalOpen(true); }}
                        style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--surface-border)', border: 'none', color: 'var(--text-primary)', fontWeight: 600, padding: '0.75rem', width: '100%', borderRadius: '8px', cursor: 'pointer', textAlign: 'left' }}
                      >
